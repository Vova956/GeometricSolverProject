...
public class SquareRootSum {
    ...
    private SumOfSquareNumbers sum;
    private int intNumber = 1;

    public SquareRootSum(SumOfSquareNumbers sum) {
        this.sum = sum.copy();
    }

    public double toDouble(){
        return Math.sqrt(sum.toDouble())*intNumber;
    }

    private boolean canDivide(double a){
        int amount = 0;
    ...
    public void getIntNumber(){
        for (int i = 0; i < squares.length; i++) {
            if(canDivide(squares[i])){
                sum.divide(squares[i]);
                intNumber *= Math.sqrt(squares[i]);
                i = -1;
            }
        }
    }
